...
{
    /**
     * @Route("/api/google-signin", name="google-Signin",  methods={"GET","HEAD", "POST" })
     */

    public function googleSignin(\Swift_Mailer $mailer ,RoleRepository $role,UserRepository $UserRepository, ?Request $request=null,UserPasswordEncoderInterface $encoder,EntityManagerInterface $em,ProjectRepository $ProjectRepository)
    {
        ...
        if($request!== null && $request->headers->get('JWT') == $API_Token )
        {
            ...
            $googleSignin = $request->request->get('googleStatus');
            $accessToken =  $request->request->get('token');
            if($usernameOrEmail !=null && $password!=null && $googleSignin!=null &&  $accessToken!=null)
            {
                $client = HttpClient::create();
                $response = $client->request('GET', 'https://www.googleapis.com/oauth2/v1/tokeninfo?access_token='.$accessToken);
                $content = $response->toArray();
                $statusCode = $response->getStatusCode();
                if($statusCode!=200)
                {
                    $loadingDataStatus = 'failure';
                    $errorStatus ='wrong token data';
                    ...
                    if($content['access_type']!=null){
                        ...
                        if($currentUserConnectEmail!=[])
                        {
                            ...
                            $timestamp = $date->getTimestamp();
                            $session_duration=$timestamp+3600;
                            $user->setSessionDuration($session_duration);
                                ...
                                { 
                                    if(isset($mariedCouple[0])===true && isset($mariedCouple[1])===true)
                                    {
                                        ...
                                        [
                                            ...
                                            'username'=>$guestProject->getUser()->getUsername(),
                                            'guestName'=>$CurrentGuestConnect[0]->getFirstname(),
    ...
    {   
     ...
     /**
     * @Route("/api/mercure/send-canvas" , name="mercure_canvas", methods={"GET","HEAD", "POST" })
     */

    public function mercureCanvas(Publisher $publisher,Request $request)
    {   
    ...
    public function mercureAnonymous(Publisher $publisher,Request $request)
    {
      ...
      /**
     * @Route("/api/project/images/upload" , name="upload_file", methods={"GET","HEAD", "POST" })
     */

    public function uploadFile(Publisher $publisher,Request $request,UserRepository $UserRepository,ProjectRepository $projectRepository)
    {
        $API_Token='eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJtZXJjdXJlIjp7InB1Ymxpc2giOlsiKiJdfX0.NFCEbEEiI7zUxDU2Hj0YB71fQVT8YiQBGQWEyxWG0po';
        ...
        if($request!== null && $request->headers->get('JWT') == $API_Token )
        {
            $date = new DateTime();
            $CurrentTimestamp = $date->getTimestamp();
            $uploadDir="/var/www/html/Owedding_BACK/O_wedding-back-end/src/Controller/uploadedFiles";
            $path="public/images/uploadedFiles";
            ...
            if($token!=null && $username != null)
            {
                ...
                $image->setSrc($unique.$name);
                $em = $this->getDoctrine()->getManager();
                $em->persist($image);
                $em->flush();
                $CurrentProject[0]->addImage($image); 
                $em->persist($CurrentProject[0]);
                $em->flush();              
                move_uploaded_file($file,"$uploadDir/".$unique.$name);
                $response = new Response();
                $downloadFileStatus ='added';
                $response->setContent(json_encode(
        
                    [  
                        
                        'images_status'=> $downloadFileStatus,
       ...
       /**
     * @Route("/api/project/images" , name="get_images", methods={"GET","HEAD", "POST" })
     */

    public function loadImages(Publisher $publisher,Request $request,UserRepository $UserRepository,ProjectRepository $projectRepository)
    {
        $API_Token='eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJtZXJjdXJlIjp7InB1Ymxpc2giOlsiKiJdfX0.NFCEbEEiI7zUxDU2Hj0YB71fQVT8YiQBGQWEyxWG0po';
        ...
        if($request!== null && $request->headers->get('JWT') == $API_Token )
        {
            ...
            if($token!=null && $username != null)
            {
                ...
                $images=[];
                $allImages = $CurrentProject[0]->getImages();
                foreach($CurrentProject[0]->getImages() as $image)
                {
                    $images[]=[    
                        'id'=>$image->getId(),
                       'name' => $image->getName(),
     ...
     /**
     * @Route("/api/project/images/delete" , name="del_images", methods={"GET","HEAD", "POST" })
     */

    public function deleteImage(Publisher $publisher,Request $request,UserRepository $UserRepository,ProjectRepository $projectRepository,ImageRepository $imageRepository)
    {
        $API_Token='eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJtZXJjdXJlIjp7InB1Ymxpc2giOlsiKiJdfX0.NFCEbEEiI7zUxDU2Hj0YB71fQVT8YiQBGQWEyxWG0po';
        ...
        if($request!== null && $request->headers->get('JWT') == $API_Token )
        {
            ...
            if($token!=null && $username != null && $imageId != null)
            {
                ...
                $imageSearch = $imageRepository->findById($imageId);
                if($imageSearch !=[] && $imageSearch[0]->getProject()->getName()==$CurrentProject[0]->getName())
                {
                   
                    
                    $em = $this->getDoctrine()->getManager();
                    $em->remove($imageSearch[0]);
                    $em->persist($CurrentProject[0]);
                    $em->flush();
                    unlink("$uploadDir/".$imageSearch[0]->getSrc());
                    $response = new Response();
                    $delImagesStatus ='deleted';
                    $response->setContent(json_encode(
            
                        [  
                            
                            'images_status'=> $delImagesStatus,
                ...
                $loadingDataStatus = 'failure';
                $errorStatus ='missing data (token/username/id)';
                $response = new Response();
                $response->setContent(json_encode(
